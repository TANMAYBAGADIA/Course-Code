...
let beatStage = 0
let effectStage = 0
let timeUntilNextBeat = 0
let globalId = 0
let spikeCheck = true
let effectCheck = true
let comunityLevel = false
let globalCId = null
let delay = 5.5
function loadMap(path) {
	delay = 5.5
	if(path == "maps/skii.json"){
		delay = 4.5
	}
...
function loadMapC(id) {
	loadGame(communityData[id])
	delay = 4.5
	comunityLevel = true
	globalCId = id
...
function loadGame(data) {
	if (gameRunning == false) {
		...
		map.texts.text.forEach(function (textData) {
			let text = new Konva.Text({ //spike drawer
				x: size + textData.x,
				y: textData.y,
				text: textData.value,
...
function createSpike() {
	/*	let spike = new Konva.RegularPolygon({ //spike drawer
			x: 1200,
			y: 80,
			sides: 3,
			radius: 15,
...
function spikeManager(current) { //Creates spikes to beat
	...
	if (current !== globalId) {
		...
		let currentTime = music.seek()
		let nextBeatTime = parseFloat(toFixed(map.beats.notes[beatStage].time, 2))
		timeUntilNextBeat = nextBeatTime - currentTime - delay //Get time until next beat -6 is how long it takes for the spike to load
		beatStage++
		...
		if (beatStage == 1 && spikeCheck && timeUntilNextBeat >= 1) { //fix insta spawn bug
			...
			timeUntilNextBeat = nextBeatTime - currentTime - delay //Get correct first beat position
...
function effectManager(current) {
	try {
		let nextEffect = map.effects[effectStage]
	} catch {
		return;
	}
	let currentTime = music.seek()
	...
	if (effectStage == 0 && effectCheck) { //fix insta spawn bug
		...
		let nextEffectTime = parseFloat(toFixed(map.effects[effectStage].time, 2))
...
function winGame() {
	...
	bossIntroCutscene(path,comunityLevel);
	setTimeout(function () {
		cancelAnimationFrame(window.gameLoop)
		cancelAnimationFrame(starAnimLoop);
	}, 1000)
...
/* Boss Battle */
// Boss info:
let boss = document.getElementById("bossDiv");

// Music Variable:
let song = null;
let timeNextBeat = null;
...
// let to check if you are playing currently: 
let inGame = true;
...
function buildGame(data) {
	...
	beatMap = tmpMap;
	let srcTmp = "maps/" + tmpMap.info.path;
...
function levelCompleted(message, starsWin) {
	...
	setTimeout(function () {
		...
		if (comunityLevel == false) {
			let nameTmp = beatMap.info.levelName.toLowerCase();
...
function calcStarScore(score) {
	let starArray = beatMap.info.starScores;
