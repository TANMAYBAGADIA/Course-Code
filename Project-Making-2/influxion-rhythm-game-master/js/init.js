...
let lastFrameRenderTime = null; //store when the last frame was drawn
let music = new Howl({
	src: ['maps/onestop.mp3']
});
let initMusic = 0 // prevent starting game on website load
let gravity = 0.5;
let keys = []
let jumpAnim
let starAnim = 50
...
let textsAnimLoop
let rockSpeedY = 5
let starAnimType = 'down'
let swordOn = false
let gameWonAlready = false;
...
let background2
let background3
let backgroundLayer = new Konva.FastLayer();
let backgroundImage = new Image();
let backgoundSpeed = 0.5;
let backgroundLoad = false
let backgroundImpactAnim
...
//Text layer
let textLayer = new Konva.FastLayer();
//Main layer Players,Stars,Spikes
let mainLayer = new Konva.FastLayer();
//Player Data
...
let sword = new Konva.RegularPolygon({
	x: 100,
	y: 530,
	sides: 3,
	radius: 20,
...
function update(time) {
	...
	if (gameRunning) {
	let timeInBetweenFrames = time - lastFrameRenderTime
